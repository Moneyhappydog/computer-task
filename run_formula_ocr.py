...
try:
    ...
    # 尝试使用真实的OCR模型
    try:
        from src.layer1_preprocessing.pix2tex_ocr import Pix2TexOCR
        print("使用pix2tex OCR模型...")
        ocr_model = Pix2TexOCR()
    except (ImportError, Exception) as e:
        print(f"⚠️ 无法使用pix2tex，使用占位OCR模型: {e}")
        print("提示: 安装pix2tex可使用真实OCR: pip install pix2tex")
    ...
    for i, r in enumerate(results[:5], 1):
        ...
        print(f"  图片: {Path(r.image_path).name}")
        print(f"  LaTeX: {r.latex[:60]!r}...")
        print(f"  检测得分: {r.detector_score:.3f}")
        if r.ocr_confidence is not None:
            print(f"  OCR置信度: {r.ocr_confidence:.3f}")
    
    if len(results) > 5:
        print(f"\n... (还有 {len(results) - 5} 个公式)")
